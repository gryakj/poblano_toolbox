function out = ncgTensor(FUN_INIT,FUN_OPT,x0,varargin)
...
if (nargin == 1) && isequal(FUN_OPT,'defaults') && (nargout == 1)
    out = params.Results;
    return;
elseif (nargin < 2)
...
%% Initialize
xk = x0;
[fk,gk] = feval(FUN_INIT,xk);
out = poblano_outTensor(xk,fk,gk,1,params);
...
while out.ExitFlag == -1
        ...
        if mod(out.Iters,params.Results.RestartIters) == 0
            ...
            switch (params.Results.Update)
                ...
                case 'HS'
                    % Hestenes-Stiefel
                    gkMgkold = gk-gkold;
                    denom = TIP(pkold,gkMgkold);
                    if denom > 0
                        bk = TIP(gk,gkMgkold)/denom;
                    else
                        fprintf(1,[mfilename,': warning: bk set to 0\n']);
                        bk = 0;
                    end
    ...
    % Compute step length
    [xk,fk,gk,ak,lsinfo,nfev] = poblano_linesearchTensor(FUN_OPT,xk,fk,gk,ak,pk,params.Results);
    ...
    out = poblano_outTensor(xk,fk,gk,nfev,params,out);    
end
